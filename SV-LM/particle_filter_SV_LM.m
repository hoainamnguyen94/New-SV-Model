function [w_T,part_T] = particle_filter_SV_LM(y,thetahat,N,Nth) 
...
%y_t = mu+lambda*h_t+eps_y_t, eps_y_t ~ N(0,e^h_t)
%h_t = mu_h+phi_h*(h_{t-1}-mu_h)+eps_h_t, eps_h_t ~ N(0,omega_h^2)
%thetahat = [mu lambda mu_h phi_h omega2_h]
...
wei = (1./exp(part(1,:)/2)).*exp(-.5*((obs(1)-lambda*part(1,:)).^2)./exp(part(1,:))); 
w(1,:) = wei/sum(wei); 
Neff(1) = 1/sum(w(1,:).^2); 
...
for t = 2:T 
    if Neff(t-1) <= Nth 
        ...
        wei = w(t-1,:).*(1./exp(part(t,:)/2)).*exp(-.5*((obs(t)-part(t,:)).^2)./exp(part(t,:))); 
    end 
    w(t,:) = wei/sum(wei); 
    Neff(t) = 1/sum(w(t,:).^2); 
end

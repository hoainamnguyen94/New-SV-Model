...
switch market
    case 1
        ...
        start_1 = 1214;
        filename = 'JCI_SV_PL.csv';
        fprintf('running SV-PL for JCI \n\n');
    case 2
        ...
        start_1 = 1236;
        filename = 'KLSE_SV_PL.csv';
        fprintf('running SV-PL for KLSE \n\n');
    case 3
        y = xlsread('ASEAN_RET.xlsx','PCOMP');
        start_1 = 1222;
        %set the name of the file to which the result will be written
        filename = 'PCOMP_SV_PL.csv';
        fprintf('running SV-PL for PCOMP \n\n');
    case 4
        y = xlsread('ASEAN_RET.xlsx','SET');
        start_1 = 1218;
        filename = 'SET_SV_PL.csv';
        fprintf('running SV-PL for SET \n\n');
    case 5
        y = xlsread('ASEAN_RET.xlsx','STI');
        start_1 = 1257;
        filename = 'STI_SV_PL.csv';
        fprintf('running SV-PL for STI \n\n');
    case 6
        y = xlsread('ASEAN_RET.xlsx','VNI');
        start_1 = 1240;
        filename = 'VNI_SV_PL.csv';
        fprintf('running SV-PL for VNI \n\n');
end
y = 100*y(1:1300); %multiply by 100
...
thetahat = SV_PL(y(1:start_5),start_5,nloop,burnin);
...
for i = 0:3
    [w_T,part_T] = particle_filter_SV_PL(y(1:(start_5+i)),thetahat,N,Nth);
    q_VAR = f_SV_PL(thetahat,w_T,part_T,N);
...
for i = 0:18
    if mod(i,5)==0
        ...
        thetahat = SV_PL(train,T_train,nloop,burnin);
    end
    [w_T,part_T] = particle_filter_SV_PL(y(1:(start_20+i)),thetahat,N,Nth);
    q_VAR = f_SV_PL(thetahat,w_T,part_T,N);
...
parfor i = 0:(iter-1) %iterate over the blocks  
    ...
    [w_T,part_T] = particle_filter_SV_PL(y(1:T),thetahat,N,Nth);
    q_VAR_1 = f_SV_PL(thetahat,w_T,part_T,N);
    [w_T,part_T] = particle_filter_SV_PL(y(1:(T+1)),thetahat,N,Nth);
    q_VAR_2 = f_SV_PL(thetahat,w_T,part_T,N);
    [w_T,part_T] = particle_filter_SV_PL(y(1:(T+2)),thetahat,N,Nth);
    q_VAR_3 = f_SV_PL(thetahat,w_T,part_T,N);
    [w_T,part_T] = particle_filter_SV_PL(y(1:(T+3)),thetahat,N,Nth);
    q_VAR_4 = f_SV_PL(thetahat,w_T,part_T,N);
    [w_T,part_T] = particle_filter_SV_PL(y(1:(T+4)),thetahat,N,Nth);
    q_VAR_5 = f_SV_PL(thetahat,w_T,part_T,N);
...
thetahat = SV_PL(train,T_train,nloop,burnin);
for i = 0:(T_total-1-T)
    [w_T,part_T] = particle_filter_SV_PL(y(1:(T+i)),thetahat,N,Nth);
    q_VAR = f_SV_PL(thetahat,w_T,part_T,N);
    start_point = 6*i+1;
    end_point = 6*(i+1);
